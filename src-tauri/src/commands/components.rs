...
pub async fn get_remote_component_versions() -> Result<ComponentManifest, String> {
    ...
    let components_json: ComponentManifest = client
        .get("https://github.com/dilllxd/StreamNook/releases/latest/download/components.json")
        .send()
        .await
...
/// as semantic versions, including this fork's `-itzon.N` pre-release suffix.
fn remote_is_newer(remote: &str, current: &str) -> bool {
    match (
        Version::parse(remote.trim().trim_start_matches('v')),
        Version::parse(current.trim().trim_start_matches('v')),
    ) {
...
/// The fork publishes this manifest beside each GitHub release. Keeping the
/// channel fork-owned prevents an upstream update from replacing Itzon builds.
const UPDATE_MANIFEST_URL: &str =
    "https://github.com/dilllxd/StreamNook/releases/latest/download/update.json";
...
async fn check_for_bundle_update_manifest() -> Result<BundleUpdateStatus, String> {
...
/// Check the fork-owned manifest first and its release assets as a fallback.
#[tauri::command]
pub async fn check_for_bundle_update() -> Result<BundleUpdateStatus, String> {
    match check_for_bundle_update_manifest().await {
        Ok(status) => Ok(status),
        Err(e) => {
            log::warn!("Fork update manifest unavailable ({e}); falling back to release assets");
            check_for_bundle_update_github().await
        }
    }
}

/// Release-asset fallback for builds published without the manifest above.
async fn check_for_bundle_update_github() -> Result<BundleUpdateStatus, String> {
    ...
    let download_url = format!(
        "https://github.com/dilllxd/StreamNook/releases/download/v{}/StreamNook.7z",
    ...
    let changelog_url = format!(
        "https://raw.githubusercontent.com/dilllxd/StreamNook/v{}/CHANGELOG.md",
        remote.streamnook.version
    );
...
mod tests {
    ...
    #[test]
    fn compares_itzon_release_versions() {
        assert!(remote_is_newer("8.5.1-itzon.2", "8.5.1-itzon.1"));
        assert!(remote_is_newer("8.5.2-itzon.1", "8.5.1-itzon.9"));
        assert!(!remote_is_newer("8.5.1-itzon.1", "8.5.1-itzon.1"));
        assert!(!remote_is_newer("8.5.1-itzon.1", "8.5.1-itzon.2"));
    }
}

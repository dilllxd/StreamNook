...
/// Compare stable and prerelease versions without offering a downgrade.
...
const UPDATE_MANIFEST_URL: &str =
    "https://github.com/dilllxd/StreamNook/releases/latest/download/latest.json";
...
/// Check for updates using the manifest attached to the fork's latest release.
    ...
    Ok(BundleUpdateStatus {
        ...
        bundle_name: Some(
            manifest
                .bundle_name
                .clone()
                .unwrap_or_else(|| format!("StreamNook-Itzon-{}-windows-x64.7z", manifest.version)),
        ),
        download_size,
...
/// Check the fork's release manifest, then fall back to its components asset.
#[tauri::command]
pub async fn check_for_bundle_update() -> Result<BundleUpdateStatus, String> {
    match check_for_bundle_update_manifest().await {
        Ok(status) => Ok(status),
        Err(e) => {
            log::warn!("Update manifest unavailable ({e}); falling back to release metadata");
...
async fn check_for_bundle_update_github() -> Result<BundleUpdateStatus, String> {
    ...
    let download_url = format!(
        "https://github.com/dilllxd/StreamNook/releases/download/itzon-v{0}/StreamNook-Itzon-{0}-windows-x64.7z",
        remote.streamnook.version,
    );
    status.bundle_name = Some(format!(
        "StreamNook-Itzon-{}-windows-x64.7z",
        remote.streamnook.version
    ));
    ...
    let changelog_url = format!(
        "https://raw.githubusercontent.com/dilllxd/StreamNook/itzon-v{}/CHANGELOG.md",
        remote.streamnook.version
    );
...
async fn install_bundle_from_status(
    ...
    // with it. The release manifest carries the hash; the fallback metadata does
    // not (sha256 = None), in which case this is skipped. A mismatch aborts
...
mod tests {
    ...
    #[test]
    fn compares_fork_prerelease_versions() {
        assert!(remote_is_newer(
            "8.4.2-itzon.2",
            "8.4.2-itzon.1"
        ));
        assert!(remote_is_newer("8.4.2", "8.4.2-itzon.2"));
        assert!(!remote_is_newer(
            "8.4.2-itzon.1",
            "8.4.2-itzon.2"
        ));

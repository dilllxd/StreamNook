...
impl ChatProvider for ItzonProvider {
    ...
    async fn release_window(&self, window: &str) {
        let mut conns = self.conns.lock().await;
        let removable: Vec<String> = conns
            .iter_mut()
            .filter_map(|(channel, connection)| {
                connection.consumers.remove(window);
                connection.consumers.is_empty().then(|| channel.clone())
            })
            .collect();
        for channel in removable {
            if let Some(connection) = conns.remove(&channel) {

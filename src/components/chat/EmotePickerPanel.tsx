...
export interface EmotePickerPanelProps {
  ...
  isTwitch: boolean;
  isKick: boolean;
  /** Non-Twitch/Kick provider whose picker consists of 7TV plus emoji/favorites. */
  supportsSevenTv?: boolean;
  channelId?: string;
  channelLogin?: string;
...
export function EmotePickerPanel({
  ...
  const defaultProvider = isTwitch ? 'twitch' : isKick ? 'kick' : supportsSevenTv ? '7tv' : 'emoji';
  const [selectedProvider, setSelectedProvider] = useState<ProviderTab>(defaultProvider);
    ...
    >
      <div className="p-2 border-b border-borderSubtle">
        ...
        <div className="flex gap-1 mt-2">
          ...
          {(isTwitch || isKick || supportsSevenTv) && (
